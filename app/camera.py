...
if __name__ == '__main__':
    ...
    ap.add_argument("-w","--worker",type=int, default=1, help="Pass the number of worker analyzing frames from queues")
    ap.add_argument("-l","--logging",default=False, help="Set to True to enable logging for multiprocessing")
    args=vars(ap.parse_args())

    appConfig.LoggingEnabled = True
    appConfig.QueueSize = args['queue_size']
    appConfig.NumWorkers = args['worker']
    appConfig.LoggingEnabled = bool(args['logging'])
    ...
    print(f"This is current config:\n{str(appConfig)} ")
